...
<div class="container-fluid">
    ...
    <div class="card border-0 shadow-sm mb-3">
        <div class="card-body py-2">
            <form action="{{ route('fasyankes.index') }}" method="GET" class="d-flex align-items-center" style="max-width: 400px;">
                <input type="text" name="search" class="form-control form-control-sm me-2" placeholder="Cari fasyankes..." value="{{ request('search') }}">
                <button type="submit" class="btn btn-sm btn-primary fw-bold"><i class="bi bi-search"></i> Cari</button>
                @if(request('search'))
                    <a href="{{ route('fasyankes.index') }}" class="btn btn-sm btn-outline-secondary ms-2 fw-bold"><i class="bi bi-x-circle"></i> Reset</a>
                @endif
            </form>
        </div>
    </div>
    ...
    <div class="card border-0 shadow-sm">
        <div class="card-body p-0">
                ...
                <table class="table table-hover align-middle mb-0">
                    <thead class="bg-teal text-white align-middle">
                        <tr>
                            ...
                            <th class="py-3">Nama Fasyankes</th>
                            <th class="py-3 text-center">Jenis</th>
                            <th class="py-3 text-center">Lokasi / Kota</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="px-4 fw-bold text-muted text-center">{{ $fasyankes->firstItem() + $index }}</td>
                            <td class="fw-bold">{{ $item->nama_fasyankes }}</td>
                            <td class="text-center"><span class="badge bg-info text-dark">{{ $item->jenis }}</span></td>
            ...
            <div class="p-3">{{ $fasyankes->links() }}</div>
        </div>
    </div>
</div>
